...
Now, if you try to add this credential, you'll get an error message saying `The agent is not online - You need to run the agent before proceeding`. You will finish creating your workspace later.

Go to the terminal, and ssh into the CS cluster and the key node (where Nextflow is supposed to run), run the tower agent in it.

To create tower connection from the CS Cluster, in your `$HOME`, download tower agent binary:
...
THIS IS FOR THE LAUNCHPAD. Change to a more appropriate directory - such as a project directory - if available, as this is going to be Nextflow's working directory if the pipeline is launched within the platform. Otherwise, use `-with-tower` and `-w` if launching from the CLI.
...
Now go to the Compute Environments section, and click on `Add compute environment`. Choose a name, select `Grid Engine` as platform, and under credentials choose the newly created credential from above.
